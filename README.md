...
| root       |  remaining space  | Linux file sys | `/mnt`      | `/dev/root_partition`       |
  ...
  فرق زیادی بین پارتیشن یا فایل بودن swap پیدا نکردم به جز اینکه تو [ویکی آرچ](https://wiki.archlinux.org/title/swap#:~:text=memory%20is%20exhausted.-,Note%3A,-The%20factual%20accuracy)
   دیدم سر این موضوع بین علما اختلاف افتاده و به نظر آقای لینوس توروالدز استفاده از پارتیشن ارحجیت داره    
  ...
  <details dir="rtl">
    <summary>برای پارتیشن بندی من از `fdisk` استفاده می‌کنم 
   ...
   اگر رم سیستم کمتر از ۲ گیگ هست یک گیگ و اگر بین ۲ تا ۴ گیگ هست ۲ گیگ و برای ۴ گیگ و بیشتر هم ۴ گیگ مقادیر معمولی هستن، ولی همونطور که گفته شد اگر ویرچوآل مموری
   ...
   - دستور `w` رو می‌زنیم و تغییرات روی دیسک رایت می‌شن و از fdisk خارج می‌شیم
